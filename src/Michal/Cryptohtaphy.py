...
# In symmetric-key encryption, the data is encoded and decoded with the same key. This is the easiest way of encryption, but also less secure. 
# The receiver needs the key for decryption, so a safe way need for transferring keys. Anyone with the key can read the data in the middle.

# Asymmetric-key Encryption:
# In Asymmetric-key Encryption, we use two keys a public key and a private key. 
# The public key is used to encrypt the data and the private key is used to decrypt the data. 
# By the name, the public key can be public (can be sent to anyone who needs to send data). No one has your private key, so no one in the middle can read your data.