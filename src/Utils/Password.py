...
class Password:
    ...
    def generate(min_length: int = 8, 
        ...
        if letters and lowercase or (not letters and lowercase):
            valid_min_length += 1
            character_types.append(ascii_lowercase)
        
        if letters and uppercase or (not letters and uppercase):
            valid_min_length += 1
        ...
        try:
            ...
            raise ValueError('Minimal lenght is not a number.')

        try:
            max_length = int(max_length)
        except:
            raise ValueError('Maximal lenght is not a number.')

        if max_length < min_length:
            raise ValueError('Maximal length cannot be lower than minnimum length.')

        if min_length < valid_min_length:
            raise ValueError('Minimal lenght of the password is too low by given criteria.')
        
        if max_length > 500:
            raise ValueError('Maximal length cannot be greather than 500.')
        
        if len(character_types) < 1:
            raise ValueError('Cannot generate password with given criteria, choose at least one type of char.')
    ...
    def validate(password: str,
        ...
        if len(password) < min_length:
            return [False, 'Password is too short. Minimum lenght is ' + str(min_length) + '.']
        
        if len(password) > max_length:
            return [False, 'Password is too long, Maximal length is ' + str(max_length) + '.']
        ...
        if (letters and not lowercase or not letters and not lowercase) and uppercase:
        ...
        if (letters and not uppercase or not letters and not uppercase) and lowercase:
            if search('[A-Z]+', password):
                return [False, 'Password does contain upper case letters.']

        if not letters and not lowercase and not uppercase:
